...
alldata['5']=dict(nomor=5, nama="andres iniesta", posisi="gelandang")
...
def proses_request(request_string):
    ...
    try:
        ...
        if (command == 'getdatapemain'):
            ...
            logging.warning("getdatapemain {}".format(nomorpemain))
            try:
...
def getConnection(koneksi, client_address, is_secure, socket_context):
    try:
        if is_secure == True:
...
def run_server(server_address, is_secure=False):
    ...
    while True:
        ...
        conn_thread = threading.Thread(target=getConnection, args=(koneksi, client_address, is_secure,socket_context))
        conn_thread.start()
